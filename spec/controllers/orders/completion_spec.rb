    ...
    describe "#postfinance" do
      ...
      it "updates order information from Postfinance request" do
        post :postfinance, params: {
          orderID: @order.main_payment.payment_id, amount: @order.main_payment.amount, STATUS: 5, PAYID: 3010824561, NCERROR: 0, SHASIGN: shaout.upcase
        }
        @order.main_payment.reload
        expect(@order.main_payment.status).to be(5)
      ...
      it "rejects a malicious request" do
        post :postfinance, params: {
          orderID: @order.main_payment.payment_id, amount: @order.main_payment.amount, STATUS: 5, PAYID: 3010824561, NCERROR: 0, SHASIGN: "random string"
        }
        @order.main_payment.reload
        expect(@order.main_payment.status).not_to be(5)
